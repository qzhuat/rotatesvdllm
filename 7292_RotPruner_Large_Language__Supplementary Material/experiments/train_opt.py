...
from lib.prune_opt import prune_wanda, prune_magnitude, prune_slice, prune_sparsegpt, check_sparsity
...
def slicing_main(args: argparse.Namespace) -> None:
    ...
    if args.prune_method == "slice":
        ...
        # Qs = [torch.eye(model_adapter.hidden_size).to(device=config.device, dtype=config.dtype) for _ in range(2*len(model.model.decoderlayers)+1)]
